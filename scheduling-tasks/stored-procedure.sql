-- inserting a record to a table, using given data as an input
CREATE OR REPLACE PROCEDURE CUSTOMERS_INSERT_PROCEDURE (CREATE_DATE varchar)
    RETURNS STRING NOT NULL
    LANGUAGE JAVASCRIPT
    AS
        $$
        var sql_command = 'INSERT INTO CUSTOMERS(CREATE_DATE) VALUES(:1);' --:1 bind holder variable for CREATE_DATE
        snowflake.execute(
            {
            sqlText: sql_command,
            binds: [CREATE_DATE]
